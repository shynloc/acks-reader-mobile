...
// window.buildThemeDoc(themeId, mdHtml, {mode, ov}) -> full HTML doc string for iframe srcdoc
...
(function () {
  ...
  const injected = {}; // spec -> true (parent document)
  function ensureGoogleFont(spec) {
    if (!spec || injected[spec]) return;
    injected[spec] = true;
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = GF_BASE + 'family=' + spec + '&display=swap';
  ...
  function gfLink(specs) {
    ...
    return `<link href="${GF_BASE}${uniq.map(s => 'family=' + s).join('&')}&display=swap" rel="stylesheet">`;
  }
...
<link href="${FONTS}" rel="stylesheet">${extra}
<style>
